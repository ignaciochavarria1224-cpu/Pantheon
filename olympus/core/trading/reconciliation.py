...
Ported near-verbatim from the stabilized old Olympus. Detection is always
read-only. Repair is allowed only for Alpaca paper mode when explicitly enabled
by settings. The broker is the truth (Article II): on any disagreement Olympus
corrects itself to match the broker.

The reconciler compares live broker state against the in-memory local position
state supplied by the caller (which is seeded from the persistent open positions
at startup), so it does not re-read the database on each cycle.
"""
...
def local_position_map(positions) -> dict[str, dict[str, Any]]:
    """Map a list of local Position objects (or dicts) to {symbol: {side, qty}}."""
    ...
    for position in positions:
        ...
        if not symbol:
            continue
        mapped[symbol] = {"side": _normalize_side(side), "qty": _normalize_qty(size)}
    return mapped
...
def detect_position_mismatch(
    ...
    if local_symbols != broker_symbols:
        ...
        return ReconciliationResult(
            local_positions, broker_positions, broker_orders,
            mismatch=True, reason=reason, entries_blocked=True,
        )
    ...
    for symbol in shared:
        if local_positions[symbol] != broker_positions[symbol]:
            return ReconciliationResult(
                local_positions, broker_positions, broker_orders,
                mismatch=True, reason="quantity_or_side_mismatch", entries_blocked=True,
            )
...
class BrokerReconciler:
    ...
    def check_and_repair(self, local_positions) -> ReconciliationResult:
        ...
        if not bool(getattr(self._settings, "OLYMPUS_AUTO_REPAIR_PAPER_POSITIONS", False)):
            return ReconciliationResult(
                result.local_open_positions, result.broker_open_positions,
                result.broker_open_orders, mismatch=True, reason=result.reason,
        ...
        logger.warning("Broker/local mismatch (%s); attempting paper auto-repair", result.reason)
        ...
        return ReconciliationResult(
            result.local_open_positions, result.broker_open_positions,
            result.broker_open_orders, mismatch=True, reason=result.reason,
            repair_attempted=True, repair_succeeded=repair_ok, entries_blocked=True,
        )
